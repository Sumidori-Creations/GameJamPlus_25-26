...
@export var radio_empuje : float = 100.0  # Aumentado para detectar mejor
...
func empujar_objetos_cercanos(direccion: Vector2):
	...
	for resultado in resultados:
		...
		# Verificar si es un objetoMovil y que no sea el propio jugador
		if objeto and objeto != self and objeto.has_method("ser_empujado"):
			# Solo empujar si está frente al jugador
			var diferencia = objeto.global_position - global_position
			var angulo_diferencia = diferencia.angle_to(direccion)
			
			# Si el ángulo es pequeño, está frente al jugador
			if abs(angulo_diferencia) < PI / 4:  # 45 grados
				objeto.ser_empujado(direccion, velocidad_empuje)
				print("¡Empujando objeto!")  # Debug
